...
const totalSum = (input) => {
  let sum = 0;
  input.map(element => {
    element.map(element => sum = element + sum);
  });
  return sum;};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let newArr = data.filter(element => element.gender === 'male' || element.gender === 'female').map(element => element.name).join(' and ');
